...
#include "RpgCameraMode_ThirdPerson.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RpgCameraMode_ThirdPerson)

namespace RpgCameraMode_ThirdPerson_Statics
{
...
URpgCameraMode_ThirdPerson::URpgCameraMode_ThirdPerson()
{
	...
	PenetrationAvoidanceFeelers.Add(FRpgPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FRpgPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FRpgPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FRpgPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FRpgPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FRpgPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
...
void URpgCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void URpgCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IRpgCameraAssistInterface* TargetControllerAssist = Cast<IRpgCameraAssistInterface>(TargetController);

	IRpgCameraAssistInterface* TargetActorAssist = Cast<IRpgCameraAssistInterface>(TargetActor);
	...
	IRpgCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IRpgCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IRpgCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IRpgCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void URpgCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IRpgCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FRpgPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(RpgCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void URpgCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void URpgCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
