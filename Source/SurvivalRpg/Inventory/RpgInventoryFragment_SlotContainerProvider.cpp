	...
	TArray<FRpgInventoryItemContainerDefinition>& OutContainers) const
{
	...
	for (const FRpgInventorySlotGroupDefinition& LegacyGroup : ProvidedSlotGroups)
	{
		...
		FRpgInventoryItemContainerDefinition& Converted = OutContainers.AddDefaulted_GetRef();
		...
		Converted.AllowedCategories = LegacyGroup.Rule.AllowedCategories;
		Converted.RequiredItemTags = LegacyGroup.Rule.RequiredItemTags;
		Converted.BlockedItemTags = LegacyGroup.Rule.BlockedItemTags;
		Converted.bQuickAccessEligible = LegacyGroup.Rule.bActionbarBindable;
		Converted.bAllowNestedContainers = false;
		Converted.MaxNestingDepth = RpgInventoryMaxItemOwnedDepth;
	}
}
