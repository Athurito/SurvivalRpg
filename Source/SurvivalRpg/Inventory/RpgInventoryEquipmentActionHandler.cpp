...
void FRpgInventoryEquipmentActionHandler::ClearEquipmentSlot(
	ERpgEquipmentSlot EquipmentSlot)
{
	URpgInventoryManagerComponent* PlayerInventory =
		FindPlayerInventory();
...
void FRpgInventoryEquipmentActionHandler::
	MoveItemToInventorySlotAddress(
		URpgInventoryItemInstance* Item,
		FRpgInventorySlotAddress TargetAddress)
{
	...
	URpgPlayerInventoryLayoutComponent* InventoryLayout =
		FindPlayerInventoryLayout();
	if (!PlayerInventory || !InventoryLayout || !Item ||
	...
	if (!InventoryLayout->ResolveSlotAddress(
			TargetAddress,
			TargetPlacement) ||
	...
	URpgInventoryItemInstance* TargetItem =
		PlayerInventory->GetItemAtContainerCell(
			TargetPlacement.GetContainerHandle(),
			TargetPlacement.X,
			TargetPlacement.Y);
	const bool bHasSourceAddress =
				...
				IsSlotContainerEquipmentSlot(SourceEquipmentSlot))
		{
				...
				InventoryLayout->GetSlotGroups())
			{
				if (Group.ContainerHandle ==
						TargetAddress.GetContainerHandle() &&
					Group.ContainsCell(
						TargetAddress.X,
						TargetAddress.Y))
				{
	...
	if (!IsSameLogicalEquipmentPlacement(
...
void FRpgInventoryEquipmentActionHandler::EquipSlotContainerItem(
	...
	URpgInventoryItemInstance* Item)
{
	...
	if (!PlayerInventory || !Item ||
		PlayerInventory->GetItemStackCount(Item) <= 0 ||
		!URpgPlayerInventoryLayoutComponent::
			IsSlotContainerEquipmentSlot(ContainerSlot))
	{
		SendActionFeedback(
			RpgGameplayTags::Rpg_Inventory_Action_Equip,
			ERpgInventoryActionFeedbackResult::InvalidRequest,
		...
		FRpgInventoryItemId ExpectedProviderItemId)
{
	...
	URpgInventoryItemInstance* ProviderItem =
		PlayerInventory->GetItemAtContainerCell(
	...
	if (!ProviderItem)
	{
		// Repair a stale presentation/loadout mirror from physical truth.
		SyncEquipmentLoadoutFromGearSlots();
		SendActionFeedback(
			RpgGameplayTags::Rpg_Inventory_Action_Equip,
			ERpgInventoryActionFeedbackResult::MissingItem,
...
void FRpgInventoryEquipmentActionHandler::EquipInventoryItem(
	URpgInventoryItemInstance* Item)
{
	URpgInventoryManagerComponent* PlayerInventory =
		FindPlayerInventory();
...
void FRpgInventoryEquipmentActionHandler::
	UnequipInventoryItemToContentSlot(
		URpgInventoryItemInstance* Item)
{
	URpgInventoryManagerComponent* PlayerInventory =
		FindPlayerInventory();
...
bool FRpgInventoryEquipmentActionHandler::
	TryBuildCurrentEquipmentIntent(
		URpgInventoryManagerComponent* Inventory,
		URpgInventoryItemInstance* Item,
