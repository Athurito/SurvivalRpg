...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RpgInventoryDragDropCoordinator)

namespace
{
	FRpgInventoryGridSize MakeInvalidSpatialSize()
	{
		FRpgInventoryGridSize InvalidSize;
		InvalidSize.Width = 0;
	...
	FRpgInventoryGridSize ResolvePayloadOccupiedSize(const FRpgInventoryDragPayload& Payload, bool bRotated)
	{
		...
		{
			return MakeInvalidSpatialSize();
		}
		return Payload.ItemFootprint.GetRotated(bRotated);
	}
	...
	FVector2D SourceVisualSize)
{
	...
	// Keep the cell-space convenience offsets aligned with the canonical anchor.
...
bool URpgInventoryDragDropCoordinator::QuickSplitEntry(URpgInventoryEntryViewModel* EntryViewModel, FRpgInventoryGridPlacement TargetPlacement, int32 SplitCount)
{
	...
	int32 ExpectedSourceQuantity = 0;
	if (!ActionComponent->CanSplitItemStack(
			Inventory,
			ItemInstance,
			SplitCount,
			TargetPlacement,
	...
	Request.ItemId = ItemInstance->GetItemId();
	Request.ExpectedEntryId = ExpectedEntryId;
	Request.ExpectedSourcePlacement = ExpectedSourcePlacement;
	Request.ExpectedSourceQuantity = ExpectedSourceQuantity;
	Request.SplitCount = ResolvedSplitCount;
	Request.TargetPlacement = ResolvedTargetPlacement;
	ActionComponent->RequestSplitItemStackById(Inventory, Request);
	return true;
}
...
bool URpgInventoryDragDropCoordinator::UseOrEquipEntry(URpgInventoryEntryViewModel* EntryViewModel, int32 StackCount)
{
	...
	const bool bEquippable =
		HasUsableDefaultEquipmentDestination(ItemInstance);
	const ERpgInventoryContextAction QuickAction =
...
bool URpgInventoryDragDropCoordinator::ExecuteEntryItemAction(
	...
	int32 StackCount)
{
	if ((Action != ERpgInventoryContextAction::Use &&
		Action != ERpgInventoryContextAction::EquipAndActivate &&
		Action != ERpgInventoryContextAction::MoveToCarry) ||
		!CanExecuteContextAction(EntryViewModel, Action))
	{
		return false;
	}
	...
	Request.ItemId = Item->GetItemId();
	Request.UseCount = FMath::Max(1, StackCount);
	Actions->RequestUseInventoryItemById(
		Inventory,
		Request);
	return true;
}
	...
	int32 StackCount)
{
	if ((Action != ERpgInventoryContextAction::Use &&
		Action != ERpgInventoryContextAction::EquipAndActivate &&
		Action != ERpgInventoryContextAction::MoveToCarry) ||
		!CanExecuteContextAction(SlotViewModel, Action))
	{
		return false;
	}
	...
	const FRpgInventoryInteractionPreviewPlan& PreviewPlan)
{
	...
	if (Target.TargetType == ERpgInventoryDropTargetType::PlayerInventorySlotAddress)
	{
		...
		{
			URpgInventoryManagerComponent* PlayerInventory = FindPlayerInventory();
			ERpgEquipmentSlot TargetHandSlot =
				ERpgEquipmentSlot::None;
			if (Payload.SourceType ==
					ERpgInventoryDragSourceType::InventoryEntry &&
				Payload.SourceInventory == PlayerInventory &&
				TryResolveHandEquipmentTarget(
					Target,
					TargetHandSlot))
			{
	...
	const FRpgInventoryDropTarget& Target) const
{
	...
	case ERpgInventoryDropTargetType::PlayerInventorySlotAddress:
	{
				...
				TargetHandSlot))
		{
			...
			Intent.Operation =
				ERpgInventoryEquipmentIntentOperation::EquipToSlot;
			Intent.TargetEquipmentSlot = TargetHandSlot;
...
FGameplayTag URpgInventoryDragDropCoordinator::ResolveActionTagForTarget(
	const FRpgInventoryDropTarget& Target,
	ERpgInventoryInteractionPreviewState AcceptedPreviewState) const
{
	return AcceptedPreviewState ==
			ERpgInventoryInteractionPreviewState::Equip ||
		Target.TargetType == ERpgInventoryDropTargetType::EquipmentSlot ||
		Target.TargetType == ERpgInventoryDropTargetType::ClearSlot
	...
	FGameplayTag ExpectedCarrySemanticRole)
{
	...
	InteractionSession->MarkRequestPending(
		Target,
		ResolveActionTagForTarget(Target, AcceptedPreviewState),
...
bool URpgInventoryDragDropCoordinator::TryResolveHandEquipmentTarget(
	const FRpgInventoryDropTarget& Target,
	ERpgEquipmentSlot& OutEquipmentSlot) const
{
	...
	return Target.TargetType ==
		...
		Target.TargetPlacement.IsValid() &&
		InventoryLayout &&
		InventoryLayout->IsCarrySlotAddress(Target.SlotAddress) &&
...
bool URpgInventoryDragDropCoordinator::BuildEquipmentIntent(
	...
	const FRpgInventoryGridPlacement& ExactTargetPlacement) const
{
	OutInventory = nullptr;
		...
		Inventory->GetAllEntries())
	{
		...
		OutIntent.TargetEquipmentSlot =
			TargetEquipmentSlot;
		OutIntent.ExactTargetPlacement =
