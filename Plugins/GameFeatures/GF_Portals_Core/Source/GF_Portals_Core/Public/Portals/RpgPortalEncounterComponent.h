 ...
 * gives each portal a deterministic technical realm-pocket transform.
 */
...
protected:
	...
	FTransform BuildRealmLevelInstanceTransform(const ARpgFeatureRegionActor& RegionActor, const ARpgFeatureEncounterPoint& EncounterPoint, int32 PortalIndex) const;

	/** Ordered rules evaluated against the owning region's ManagedEncounterPoints. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Portal|Encounter")
	TArray<FRpgPortalEncounterSpawnRule> SpawnRules;

	/** Hidden world-space origin for technical realm level-instance pockets. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Portal|Realm", meta = (ForceUnits = "cm"))
	FVector RealmPocketBaseLocation = FVector(1000000.0, 0.0, 0.0);

	/** World grid size used to bucket overworld regions into deterministic realm-pocket ranges. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Portal|Realm", meta = (ClampMin = "1000.0", ForceUnits = "cm"))
	double RealmPocketRegionGridSize = 100000.0;

	/** Distance between realm-pocket ranges for neighboring region buckets. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Portal|Realm", meta = (ClampMin = "1000.0", ForceUnits = "cm"))
	double RealmPocketRegionStride = 1000000.0;

	/** Distance between individual portal realm instances inside one region bucket. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Portal|Realm", meta = (ClampMin = "1000.0", ForceUnits = "cm"))
	double RealmPocketPortalSpacing = 100000.0;
