...
- The complete 77-sequence `Stand.Run` family uses manifest profile `rpg_no_leg_source_blend_v1`: the stock GASP UE5.8 retarget graph with `Blend to Source=0.0` for `LeftLeg` and `RightLeg`. The source and Manny thigh/calf proportions are non-uniform, so the stock value `1.0` forces the downstream Full Body IK pass to full leg reach. Run IK, Speed Plant, pelvis handling, goal offsets, arm chains, curves, native Pose Search metadata, and the project-curated root tracks remain intact.
- The rollout covers all 71 Sparse runtime members plus the six legacy-only Run clips, so Motion Matching and transition blends never mix incompatible leg-retarget profiles. The original video-traced six-clip pilot remains the strict knee-angle regression boundary; full-family behavior stays a runtime visual acceptance gate.
- `CuratedAssetManifest.csv` records the explicit rollout profile on every `Stand.Run` row, and the content contract requires exact 77/77 manifest-to-asset coverage so a future reimport cannot silently restore the faulty setting.
