...
- The issue #99 forward-run pilot (`Loop_F`, both forward starts, and both backward-to-forward pivots) uses manifest profile `rpg_no_leg_source_blend_v1`: the stock GASP UE5.8 retarget graph with `Blend to Source=0.0` for `LeftLeg` and `RightLeg`. The source and Manny thigh/calf proportions are non-uniform, so the stock value `1.0` forces the downstream Full Body IK pass to full leg reach. Run IK, Speed Plant, pelvis handling, goal offsets, arm chains, curves, and native Pose Search metadata remain enabled and source-derived.
...
- `CuratedAssetManifest.csv` records `gasp_ue5_8_stock` on unchanged assets and the explicit override on the five pilot rows, so a future reimport cannot silently restore the faulty leg-chain setting.
