...
- The issue #99 video-traced forward-run pilot (`Loop_F`, both forward starts, both backward-to-forward pivots, and `Reface_Start_F_R_090`) uses manifest profile `rpg_no_leg_source_blend_v1`: the stock GASP UE5.8 retarget graph with `Blend to Source=0.0` for `LeftLeg` and `RightLeg`. The source and Manny thigh/calf proportions are non-uniform, so the stock value `1.0` forces the downstream Full Body IK pass to full leg reach. Run IK, Speed Plant, pelvis handling, goal offsets, arm chains, curves, and native Pose Search metadata remain enabled and source-derived. The Reface clip extends the A/B to the concrete mixed-profile transition observed in runtime footage; its isolated knee-angle guard is a regression boundary, while the repeated in-game transition remains the visual acceptance gate.
- The remaining `Stand.Run` slice is intentionally unchanged in this pilot. It must use the same leg-chain profile if the visual acceptance pass expands the fix, so Motion Matching does not mix incompatible retarget profiles.
- `CuratedAssetManifest.csv` records `gasp_ue5_8_stock` on unchanged assets and the explicit override on the six pilot rows, so a future reimport cannot silently restore the faulty leg-chain setting.
