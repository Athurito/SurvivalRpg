...
- The profile also owns the relaxed unarmed upper-body posture targets for Idle, Walk, and Run plus their cosmetic interpolation rate. `URpgAnimInstance` advances only a value-type rotation from the existing CMC gait snapshot during parallel update; the pilot AnimGraph applies it additively to `spine_01` after `DefaultSlot` and before Foot Placement/Leg IK. Montage, crouch, airborne, and combat-overlay gates remain downstream graph policy, while pelvis/root and physical movement stay untouched.
