...
- All 190 curated sequences use manifest profile `rpg_no_leg_source_blend_v1`: the stock GASP UE5.8 retarget graph with `Blend to Source=0.0` for `LeftLeg` and `RightLeg`. The source and Manny thigh/calf proportions are non-uniform, so the stock value `1.0` can force the downstream Full Body IK pass to full leg reach. The original 77-clip Run fix is retained; the same correction now covers Walk, Sprint, Crouch, turns, airborne and landing families. Run IK, Speed Plant, pelvis handling, goal offsets, arm chains, curves, native Pose Search metadata, and the project-curated root tracks remain intact.
- `RetargetPoseReference.csv` records source UEFN knee metrics at 60 Hz, including the last key, from the same local GASP revision documented above. `FullFamilyKneePoseContract` evaluates every project clip on Manny in Raw and Compressed form after derived-data compilation. It permits source-authored straight legs with a 2-degree maximum-angle allowance and at most 0.02 additional near-straight frame fraction (either knee >=175 degrees). The stricter six-clip Run guard remains in place. These are asset-pose contracts; final AnimGraph blends, skinning and terrain IK still need rendered acceptance.
- `CuratedAssetManifest.csv` requires exact 190/190 manifest-to-asset coverage. `Scripts/retarget_curated_locomotion.py` provides selective dry-run/apply authoring through the editor-only `URpgAnimationRetargetLibrary`: only non-root bone tracks are copied from validated candidates. Its native metadata signature includes complete rich curves, typed attribute values, notify events and owned notify settings, including protected fields unavailable through Python. Existing settings, root tracks and package identities are preserved; sample and staging assets are never saved.
...
- The profile also owns the relaxed upper-body posture targets for Idle, Walk, and Run plus their cosmetic interpolation rate. `URpgAnimInstance` advances only a value-type rotation from the existing CMC gait, rotation-mode, and fallback snapshots during parallel update. The relaxed baseline remains active for Free locomotion and whenever fallback presentation owns the upper body, but decays to zero when an authored combat profile resolves successfully in CombatStrafe/Aim so that profile keeps ownership. The pilot AnimGraph applies the effective rotation additively to `spine_01` after `DefaultSlot` and before Foot Placement/Leg IK; montage, crouch, and airborne gates remain graph policy, while equip/profile crossfades cannot pulse the correction. Pelvis/root and physical movement stay untouched.
- The worker-safe Foot Placement release retains GASP's relative foot correction on the live FK pose while it fades. It never holds a released rear foot at the previous absolute world position; game-thread traces and value-only snapshots remain unchanged.
...
The prediction-owned desired gait enters Run at input `>= 0.70`, retains Run at `>= 0.65`, and exits to Walk only below `0.65`. One CharacterMovement SavedMove `Custom0` flag stores that gait through authoritative movement and client replay. The synchronous SavedMove solve preserves that decision with a bounded Quantize10 error allowance for acceleration transformed through a rotated movement base; it does not reclassify the move across a differently rounded hysteresis boundary. Low-input Run requests remain invalid. The authority publishes current standing Walk/Run via `GroundMovementGait` with `COND_SimulatedOnly`, both during active input and coast. New, newly relevant or previously out-of-date proxies consume current server gait before local history, including active input in the 0.65–0.70 retention band. The value clears at physical stop or when leaving standing ground movement; no pose or AnimBP history is replicated.

Server `ForcePositionUpdate` extrapolation reuses the last validated gait with that same rounding allowance while owner moves are missing. This scope ends with the forced solve, so subsequent local input continues to make normal gait decisions.

Controller-facing-to-Free transitions use an owner-only authority revision and a reliable acknowledgement of the last move authored before the owner applied Free. The authority then sends a normal timestamped CMC rotation correction for a strictly newer move, retrying until receipt is confirmed. CMC restores yaw before replay; stale revisions and superseded transitions are ignored. This also converges after input is released during the mode's replication delay.
