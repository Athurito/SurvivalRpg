...
- **In Arbeit: `GASP-02` / `GASP-NET-02` – terminaler Mantle-Grund bei Reconciliation.** Branch `codex/gasp-net-02-terminal-reconciliation`, Basis `44a5e512`. Ursache für vorhergesagtes `Finished` gegenüber autoritativem `Cancelled` und zulässigen Terminalvertrag prüfen; noch kein Fix oder neuer Validierungserfolg behauptet.
...
| `GASP-02` | Gezielte Stabilisierung | **In Arbeit (Teilauftrag)** | `GASP-STAB-01` gemergt in PR #145; `GASP-STAB-02` gemergt in PR #146; `GASP-NET-01` gemergt in PR #147; `GASP-NET-02` in Ursachenprüfung, weitere Registerpunkte offen |
...
gemergt; `GASP-NET-02` ist in Arbeit, die übrigen Punkte sind offen.**
...
| `GASP-NET-02` | **In Arbeit**, Ursachenprüfung | Ein Lauf zeigte vorhergesagtes `Finished` gegenüber autoritativem `Cancelled` bereits vor der Positionsinjektion. Cleanup-Grund und zulässige terminale Semantik frisch nachweisen. Ein Wiederholungslauf mit gleichem Grund auf beiden Seiten löst diese Beobachtung nicht; noch kein NET-02-Fix oder Validierungserfolg. |
